...
def test_application_draft_prompt_separates_matching_keywords_from_display_skills() -> None:
    ...
    assert "French and English: 250-350 words" in prompt
    assert "Use projects, stacks or experiences as high-level proof only" in prompt
    assert "Name at most one project in the motivation letter" in prompt
    assert "do not mention SciFact RAG Verifier in the motivation letter" in prompt
    assert "selected_project_ids must contain at least 4 projects" in prompt
...
def test_application_draft_prompt_adds_langchain_langgraph_letter_note_only_when_requested() -> None:
    profile = get_profile()
    base_kwargs = {
        "profile": profile,
    ...
    prompt = application_draft.build_user_prompt(
        analysis=_sample_analysis().model_copy(
            update={
                "main_tasks": ["Build agentic workflows with LangGraph"],
                "required_skills": ["Python"],
            }
        ),
        **base_kwargs,
    )
    assert "LangChain/LangGraph note (motivation letter only)" in prompt
    assert "Text-to-SQL assistant project soon" in prompt
    assert "is still deepening them" in prompt
    assert "do not mention LangGraph" in prompt

    prompt_without_tools = application_draft.build_user_prompt(
        analysis=_sample_analysis().model_copy(
            update={
                "main_tasks": ["Build data science pipelines"],
                "required_skills": ["Python"],
                "cv_keywords_to_include": ["PyTorch"],
            }
        ),
        **base_kwargs,
    )
    assert "LangChain/LangGraph note" not in prompt_without_tools

    prompt_with_langchain = application_draft.build_user_prompt(
        analysis=_sample_analysis().model_copy(
            update={
                "main_tasks": ["Build RAG applications with LangChain"],
                "required_skills": ["Python", "LangChain"],
            }
        ),
        **base_kwargs,
    )
    assert "LangChain/LangGraph note (motivation letter only)" in prompt_with_langchain
    assert "In CV fields, mention LangChain only if it is selected from allowed_skills" in (
        prompt_with_langchain
    )
    assert "LangChain" in prompt_with_langchain
