...
class LinkedInOfferAdapter:
    """Translate LinkedIn source payloads into shared offer contracts."""
    ...
    def build_filter_facts(self, source_data: dict[str, Any] | None):
        ...
        from smartapply.filtering.source_fact_builders.linkedin import (
            build_linkedin_filter_facts,
        )

        if not isinstance(source_data, dict):
            return FilterFacts(source=self.source)
        return build_linkedin_filter_facts(source_data)
    ...
    def build_analyzer_metadata(self, source_data: dict[str, Any] | None) -> str:
        ...
        return build_linkedin_source_metadata(source_data)
